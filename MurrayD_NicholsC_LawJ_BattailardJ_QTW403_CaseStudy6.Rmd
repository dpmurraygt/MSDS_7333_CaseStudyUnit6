...
Applications for RTLS are vast and extend beyond a simple enclosed space. Advances in wireless technologies and proliferation of tracking tags have made real time location systems ubiquitous in manufacturing, inventory management and navigation [3]. Current location identification of a package is a popular use of RTLS technology. However, tracking previous location history and tracing locations to predict future locations are also common tasks, especially in inventory management and logistics. Active tags can provide critical data such as the temperature of a package or the blood sugar level of a patient along with location information.

In this paper, we extend previous analysis by Nolan and Temple Lang [2] using indoor wireless signal strength data from Mannheim University. These data are generated from a single mobile device at 166 locations on a single floor of a building. Eight orientation angles are considered at location and 110 readings are taken for each (x,y) location, angle combination. Seven access points provide signal strength data to the mobile device. 

In their analysis, Nolan and Temple Lang implement an indoor positioning system using an average-based k nearest neighbors model. The model is used to predict the location of a mobile device on the same floor using previously unseen signal strength data. The authors drop an access point from their data set and use six of the seven access points to build their K-nearest neighbors model. We investigate the accuracy of the authors' K-nearest neighbors model utilizing the access point previously not considered. We also extend the average-based K-nearest neighbors approach by considering a weighted K-nearest neighbors model for predicting the location of the mobile device.

## Literature Review
Real-time location systems have no shortage of literature to enhance one’s knowledge on the subject.  There are a multitude of techniques for RTLS to work, from radio frequency identification (RFID), to infrared laser line-of-sight systems and ultrasound systems as the more common ones.  These systems allow for location data to be passed through objects, as in RFID, or provide more precise indoor locations in the case of ultrasound or line-of sight systems.  The techniques to analyze the position of the identification tags with each system can vary as well, and though we utilized k-nearest neighbors, other systems are used depending on the communication protocol used for the tags. [5]

There are multiple uses for such technology, from inventory management to livestock tracking or even patient and worker tracking.  Healthcare has continued to work on ways to utilize such technology while limiting cross-contamination, such as in the Toronto University Health Network.  This system allows the university to locate personnel and equipment entering a contaminated area and then sends an alert if the person or equipment hasn't been decontaminated or goes to a loction that could introduce cross-contamination. [8] Or a patent filed for tracking livestock that would assist ranchers and farmers in keeping location information on their herds.  This system using the distance value transmitted from the reader to the system which then utilized k-nearest neighbor analysis to determine location. [7]

One of the bigger challenges with RTLS systems and current technologies is patient or worker confidentiality.  Something that isn’t taken into account in the assignment, but is still a significant privacy concern for most businesses and which makes true RTLS in a business setting where people are involved less than appealing.  One solution for that was proposed in Yi, et al, where they propose to utilize the Paillier public-key cryptosystem in the kNN query of the location.  This provides the end-user with both location and data privacy.  This is strictly in a more request oriented type, however reversing such technology for use in business can be achieved while maintaining a person’s privacy and are proven to be more efficient than current methodologies. [9]

While not directly related to the assignment, the research into such methodologies shows that there is much more that can be done with the kNN back-end in RTLS methodologies than we can outline in one paper.  The technology will continue to move forward, with businesses using RTLS to improve fulfillment requests using robotics, such as Amazon[3], or in inventory management and warehouse distribution in a company[6].  Such systems are in wide use in business, and even some defense contractors utilize the technology to assist in moving products from one location to another in their industrial processes to increase safety, efficiency and reduce human overhead.[10]  Further discussion will show our methods and how we tackle the issues that are presented in the Nolan and Lang.

## Methods

Prior to analysis, extensive data preparation was undertaken to transform the raw data. Before discussing methods for accomplishing our two analysis objectives, we provide salient findings about the raw training dataset and the transformations taken to prepare the data for analysis.

The raw training data contains 151,392 unique lines detailing the (x,y) position and orientation of a mobile device. At each position and orientation combination, 110 readings are taken from a single mobile device. Each reading contains signal strength values from access points around the target building floor as well as extraneous floors. In addition to signal strength readings, variables such as the POSIX time and the physical MAC address for each signal are also captured.

After excluding extraneous signals, normalizing orientation values and translating time variables, a dataset for data analysis is created. The data set format can be seen in Table 1.

```{r offline_sample, include=TRUE, echo=FALSE, fig.cap="Table 1: Transformed Offline Dataset Structure"}
...
After transforming the data, key exploratory data analysis results help to form assumptions and methods for an indoor positioning system. Non-normal, multi-modal signal distributions are found when considering signal strength and the angle of the mobile at each (x,y) location reading. This indicates angle has an effect on signal strength. Figure 2 displays this finding for one MAC address at an (x,y) position of 24,4.

```{r angle_density_plot, include=TRUE, echo=FALSE, fig.cap="Figure 1: Angle Based Density Plots for 00:0f:a3:39:e1:c0"}
...
The transformed dataset is subsequently aggregated by position, angle and MAC address. Additional analysis using the aggregated data shows higher variability for stronger signal strengths. Further, analysis of the relationship between signal and distance confirms that signal strength weakens as distance increases. This negative relationship can be seen for each access point in Figure 2.

```{r offlineSummary_creation, include=FALSE, echo=FALSE}
...
```{r xyplot, include=TRUE, echo=FALSE, fig.cap="Figure 2: Relationship Between Signal and Distance"}
...
Of particular note is orientation's effect on signal strength.  Nolan and Temple Lang also drop the access point with a MAC address of 00:0f:a3:39:dd:cd. We will explore the impact of the missing MAC address and orientation's effect on our k nearest neighbors model in the next two sections.
...
# talk about how aggregate set is used and casted to form KNN dataset

Nolan and Temple Lang [2] implement a k nearest neighbors model utilizing the mean of known neighbors' (x,y) location to predict the position of new records. These new records contain signal strengths from six access points on the building floor. Prior to model fitting, it is determined that the orientation of a reading has an impact on signal strength. For instance, when considering an x,y position of (2,12), MAC address 00:14:bf:b1:97:90 shows signal strength variability differences when comparing orientations. 
...
Nolan and Temple Lang [2] identify an angle parameter when creating their k nearest neighbors model. This parameter allows the model to utilize a number of angles when establishing a training set for predicting the location of new observations. While we do not explore the angle parameter in our analysis, we do note that Nolan and Lang consider three angles when building the training set used for prediction in their k nearest neighbor model. For consistency, we maintain three angles in our implementation of a weighted k nearest neighbors model. This allows for multiple angles to influence the mean signal strength used for each observation in our training dataset.
...
```{r data_prep, include= FALSE, echo=FALSE, eval=FALSE}
...
```{r data_prep_func, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r eda, eval=FALSE, echo=FALSE, include=FALSE}
