...
Applications for RTLS are vast and extend beyond a simple enclosed space. Advances in wireless technologies and proliferation of tracking tags have made real time location systems ubiquitous in manufacturing, inventory management and navigation [3]. Current location identification is a popular use of RTLS technology. However, tracking previous location history and tracing locations to predict future locations are also common tasks, especially in inventory management and logistics.

In this paper, we extend previous analysis using indoor wireless signal data from Monneheim (sp) University [2]. These data are generated from a single mobile device and seven access points (readers) at 166 locations on one floor of a building. In previous analysis, Nolan and Temple Lang implement a K-nearest neighbors approach using six of seven access points, dropping an access point that was in a similar location to another access point. We investigate the effectiveness of this K-nearest neighbors model utilizing the access point previously not considered. We also extend the K-nearest neighbors approach by considering a weighted K-nearest neighbors implementation. 
...
sapply(list(estXYk1, estXYk3, estXYk3wtd))
